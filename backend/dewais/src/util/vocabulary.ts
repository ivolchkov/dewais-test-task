const mapVocabulary = (): [string, string][] => {
  const vocabulary = {
  ...
  return Object.entries(vocabulary).reduce((acc, item) => {
    const [type, words] = item;

    words.forEach(word => acc.push([word, type]));

    return acc;
  }, [] as [string, string][]);
};

export const vocabularyMap: Map<string, string> = new Map(mapVocabulary());